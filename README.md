...
### 今後追加したい機能・技術
- Youtube・ストリーミングサイトへのリンク追加
- アーティスト・アートワークに対応したネット記事のリンク追加
- レイアウト追加(情報の見せ方を変える、例えばアートワークをタイムフローように縦に並べるなど)
...
### 1. パフォーマンス改善
  ...
  そのため、`django-debug-toolbar`や標準ログライブラリで発行SQLで確認しつつ、1+N問題を解消しました。
  ...
  具体的には、
      * 一度必要データ分を全件手元に取得する→手元データ内で登録有無を確認  
	  (HerokuDBへの問い合わせが不要)
      * 通常IDとは別に各データにユニークキーのカラムを設ける
      * `bulk_create()`のオプション`ignore_conflicts=True`を使用する  
	  (データをSQL1件で一括登録できるかつユニークキーで重複がある場合はDBへの登録をしない)
